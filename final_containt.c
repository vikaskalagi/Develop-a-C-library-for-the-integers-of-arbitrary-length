...
{	//printf("df");
	...
	{//printf("%cr%d ",a[i],c);
		...
		{	if(a[i]=='0')
			...
			result[k++]=a[i]-c;
			//sprintf(&result[k++], "%d", a[i]-'0'-c);
			c=0;
		}
		}
		else
		{	result[k++]=a[i];
			//sprintf(&result[k++], "%d", a[i]-'0');
		}
		i--;
	}
	//printf("%s zxcv\n",result );
...
char* intal_multiply(const char* intal1, const char* intal2)
{
    ...
    { 
        ...
        { 
            ...
            int sum ; 
  			//printf("before:%c %d %s\n ",result[i_n1 + i_n2],i_n1+i_n2,result);
            ...
            //sprintf(&result[i_n1 + i_n2], "%d", sum%10);
...
char* intal_mod(const char* intal1, const char* intal2)
{
	...
	int l2=strlen(intal2);
	temp[0]=(char *)malloc(sizeof(char)*l2);
	strcpy(temp[0],intal2);
	//printf("%s ty]\n",temp[0]);
	...
	{	
		sprintf(&a[0], "%d", i);
		a[1]='\0';
		temp[i-1]=intal_multiply(intal2,a);

	}
	temp[9]=intal_multiply(intal2,"10");
	free(a);
	//printf("%s\n", temp[9]);
	int l1=strlen(intal1);
	...
	char *b;
	b=(char *)malloc(sizeof(char)*(l2+2));
	int k=0;
	if(l1>=l2)
	{memcpy(b, &intal1[0], l2 );
	b[l2]='\0';}
	if(l1<l2)
	{
		char *result=malloc(sizeof(char)*l1);
	...
	while(k<=l1)
	{	//printf("%s %s tyubn \n",b,temp[cou]);
		int z=intal_compare(b,temp[cou]);
		//printf("%d io\n",z);
		if(z==0)
		{
			if(k==l1)
		...
		else if(z==1)
		{
			...
			//printf("%s %s rty\n",b ,intal1);
			//printf("dsdfdfa");
			while(intal_compare(b,temp[opi+1])==1 && (opi+1)<10){
				opi++;//printf("%d\n", opi);
			}

			strcpy(b,intal_diff(b,temp[opi]));
			//printf("%s %s rty\n",b ,intal1);
			int rp=strlen(b);
			if(l1-k<(l2-rp))
			{
				memcpy(&b[rp], &intal1[k], l1-k );
				b[l1-k+rp]='\0';
				//printf("df\n");
				return b;
			}
			...
			{
				memcpy(&b[rp], &intal1[k], l2-rp );
				b[l2]='\0';
				k+=(l2-rp);
			}
			cou=0;
			//memcpy(&b[rp],&intal1[rp])
		}
		else
		{
			//cou++;
